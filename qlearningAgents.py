...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        for a_next in actions:
          qvec.append(self.qvalues[state,a_next])
        maxQ=max(qvec)
        return maxQ
    ...
    def update(self, state, action, nextState, reward):
        ...
        self.qvalues[state,action]=self.getQValue(state,action)*(1-self.alpha) +self.alpha*(reward+self.discount*maxQ)
        ...
        for key in features:
          ...
          if len(self.getLegalActions(nextState))==0:
            ...
            for nextAction in self.getLegalActions(nextState):
              qlist.append(self.getQValue(nextState, nextAction))

            maxQ=max(qlist)
            difference=(reward+self.discount*maxQ)-self.getQValue(state,action)
          # print('diff',difference)
          
          factor=(self.alpha*difference)
          self.weights[key]= w[key]+factor*features[key]
